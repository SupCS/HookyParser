...
Команда собирает все 10 локаций, сохраняет отдельную дневную запись для каждой и завершается. Повторный сбор текущего дня перезаписывает его данные; прошедшие дни остаются неизменными. При отсутствии `DATABASE_URL` используется `hooky_history.sqlite3`.
