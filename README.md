...
Команда собирает все 10 локаций на сегодня и шесть следующих дней через GraphQL Hooky, сохраняет отдельную дневную запись для каждой даты и завершается. Повторный сбор текущих и будущих дней перезаписывает их данные; прошедшие дни остаются неизменными. Горизонт можно изменить переменной `HOOKY_FUTURE_DAYS` (от 0 до 31). При отсутствии `DATABASE_URL` используется `hooky_history.sqlite3`.
